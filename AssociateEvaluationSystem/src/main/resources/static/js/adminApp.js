...
adminApp.controller('RegisterEmployeeCtrl', function($scope,$mdToast,$location,$http,SITE_URL, API_URL, ROLE) {
	...
	$scope.initializeRegistrationSelects = function() {
		...
		.then(function(result) {
			var tempScope = [];
			var temp  = result.data._embedded.roles;
			...
			//$scope.roleTypes = ;
			$scope.roleTypes = tempScope;
			
		/*	// intended to capitalize role titles for view, dont want to change actual value/ come back to this
		 * $scope.roleTypes.forEach(function(role) {
				role.roleTitle = role.roleTitle.charAt(0).toUpperCase() + role.roleTitle.slice(1);
			}); 	*/ 
...
//				if ($scope.roleTypes.roleTitle.toUpperCase() === 'CANDIDATE'){
