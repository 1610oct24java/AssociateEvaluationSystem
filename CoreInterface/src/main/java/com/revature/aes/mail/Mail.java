...
import com.revature.aes.service.MailService;
...
public class Mail {
		...
		public void sendEmail(MailObject m, String email){
			...
			case "candidateNeedsQuiz":
				...
				ms.sendEmail(email, "Revature Quiz", CANIDATE_COMPLETED_BODY + "Link: " + m.getLink() 
						+ "\nTemporary Pass: " + m.getTempPass());
				break;
				
			case "candidateNotCompleted":
				ms.sendEmail(email, "Quiz Timer Expired", CANDIDATE_NOT_COMPLETE_BODY);
				ms.sendEmail(recruiterEmail, candidateName + " did not complete their quiz", "The timer for: " + candidateName + " has expired and their temporary password is no longer invalid.");
				break;
			
			case "candidateCompleted":
				int grade = ad.findByUser(candidate).getGrade(); 
				ms.sendEmail(recruiterEmail, candidateName + " has completed quiz", candidateName
